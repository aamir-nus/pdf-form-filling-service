...
- Yes/No (option-pick) fields are radio groups — they render as clickable boxes grouped by field, so picking one option clears the others. Independent checkboxes ("click if yes", "check all that apply") stay separate. Dropdowns (combo/list) render as `<select>`.
- Downloaded files are named `<original>_filled_<HH:MM:SS><DD-MM-YYYY>.pdf`.
